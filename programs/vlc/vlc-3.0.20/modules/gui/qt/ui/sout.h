...
public:
    ...
    {
        Sout->setWindowTitle(Q_("Stream Output", nullptr));
        sourceWizardPage->setTitle(Q_("Source", nullptr));
        sourceWizardPage->setSubTitle(Q_("Set up media sources to stream", nullptr));
        inputBox->setTitle(QString());
        destinationWizardPage->setTitle(Q_("Destination Setup", nullptr));
        destinationWizardPage->setSubTitle(Q_("Select destinations to stream to", nullptr));
        label_4->setText(Q_("Add destinations following the streaming methods you need. Be sure to check with transcoding that the format is compatible with the method used.", nullptr));
        label_3->setText(Q_("New destination", nullptr));
        addButton->setText(Q_("Add", nullptr));
        localOutput->setText(Q_("Display locally", nullptr));
        destTab->setTabText(destTab->indexOf(tab), QString());
        wizardPage->setTitle(Q_("Transcoding Options", nullptr));
        wizardPage->setSubTitle(Q_("Select and choose transcoding options", nullptr));
        transcodeBox->setText(Q_("Activate Transcoding", nullptr));
        optionsWizardPage->setTitle(Q_("Option Setup", nullptr));
        optionsWizardPage->setSubTitle(Q_("Set up any additional options for streaming", nullptr));
        groupBox_3->setTitle(Q_("Miscellaneous Options", nullptr));
        soutAll->setText(Q_("Stream all elementary streams", nullptr));
        groupBox_4->setTitle(Q_("Generated stream output string", nullptr));
    } // retranslateUi
